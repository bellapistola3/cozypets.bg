...
const BookingModal: React.FC<BookingModalProps> = ({ sitter, onClose, onBookingComplete }) => {
  ...
  const [formData, setFormData] = useState<any>({
    serviceType: (sitter as any).services?.[0] || 'daily-walks',
    startDate: '',
    endDate: '',
    startTime: '09:00',
    endTime: '17:00',
  ...
  const loadUserPets = async () => {
    if (!user) return;
    try {
      const data = await dbHelpers.getUserPets(user.id);
      setPets(data as any[]);
    } catch (err) {
  ...
  const calculateTotal = () => {
    ...
    if (!formData.startDate || !formData.endDate) return defaultPricing;
    ...
    const servicePrice = s.pricing?.[formData.serviceType] || 0;
        ...
        <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl">
          <div className="flex items-center justify-between p-6 border-b">
            <h2 className="text-2xl font-bold text-gray-900">
              Резервация при {(sitter as any).firstName || (sitter as any).name}
            </h2>
            <button
              onClick={onClose}
          ...
          <div className="p-6">
            ...
            <div className="flex items-center justify-center mb-8">
              <div className="flex items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${step >= 1 ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-600'
                  }`}>
                  1
                </div>
                <div className={`w-16 h-1 ${step >= 2 ? 'bg-green-600' : 'bg-gray-200'}`}></div>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${step >= 2 ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-600'
                  }`}>
                  2
                </div>
                <div className={`w-16 h-1 ${step >= 3 ? 'bg-green-600' : 'bg-gray-200'}`}></div>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${step >= 3 ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-600'
                  }`}>
                  3
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData((prev: any) => ({ ...prev, serviceType: e.target.value as any }))}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500"
                  >
                    {((sitter as any).services || []).map((service: any) => (
                      <option key={service} value={service}>
                        {getServiceLabel(service)} - {(sitter as any).pricing?.[service] || (sitter as any).hourly_rate} лв.
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData((prev: any) => ({ ...prev, petId: e.target.value }))}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500"
                    required
                  >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setFormData((prev: any) => ({ ...prev, specialInstructions: e.target.value }))}
                    rows={4}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500"
                ...
                <div className="bg-gray-50 rounded-lg p-6">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span>{pets.find((p: any) => p.id === formData.petId)?.name}</span>
                    </div>
                  </div>
                </div>
