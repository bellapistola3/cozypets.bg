...
const PremiumMagazine: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    if (user) {
      const sitterProfile = await dbHelpers.getSitterProfile(user.id);
      if (sitterProfile && (sitterProfile as any).is_premium) {
        // Simple check for premium status
        setIsPremium(true);
      }
    }

    const issueData = await dbHelpers.getLatestMagazineIssue();

    if (issueData) {
      setCurrentIssue(issueData);
      const articlesData = await dbHelpers.getMagazineArticles(issueData.id);
      setArticles(articlesData);
    }

    const clinicsData = await dbHelpers.getVeterinaryClinics();
