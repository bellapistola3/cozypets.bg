...
const VeterinaryChat: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (activeChatRoom) {
      ...
      unsubscribe = dbHelpers.subscribeToChatMessages(activeChatRoom.id, (newMessages) => {
  ...
  const loadChatRooms = async () => {
    if (!user) return;
    const data = await dbHelpers.getVetChatRooms(user.id);
    setChatRooms(data as any || []);
  };

  const loadVeterinarians = async () => {
    const data = await dbHelpers.getVeterinarians();
    const vets = data?.map((vet: any) => ({
      id: vet.id,
      profile_id: vet.profile_id,
      full_name: vet.full_name || vet.profiles?.full_name || 'Unknown',
      avatar_url: vet.avatar_url || vet.profiles?.avatar_url,
  ...
  const loadMessages = async (chatRoomId: string) => {
    const data = await dbHelpers.getChatMessages(chatRoomId);
  ...
  const createNewChat = async () => {
    ...
    try {
      const chatRoom = await dbHelpers.createVetChatRoom({
        user_id: user.id,
        subject: newChatForm.subject,
        urgency_level: newChatForm.urgency_level,
        pet_id: newChatForm.pet_id || undefined,
      });
      ...
      loadChatRooms();
      setActiveChatRoom(chatRoom as any);
    } catch (error) {
              ...
              {chatRooms.length === 0 ? (
                ...
                chatRooms.map((room) => (
                  <div
                    ...
                    className={`p-4 cursor-pointer hover:bg-gray-50 transition-colors ${activeChatRoom?.id === room.id ? 'bg-green-50' : ''
                      }`}
                  >
