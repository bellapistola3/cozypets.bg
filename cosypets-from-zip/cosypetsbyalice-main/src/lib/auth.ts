...
export const authHelpers = {
    ...
    try {
      ...
      if (user) {
        try {
          ...
          console.error('Error creating user profile in Firestore:', userError);
          // Delete the newly created auth user if profile creation fails? 
          // For now, at least throw the error so the UI knows registration isn't "done"
          throw new Error(`Auth success, but profile creation failed: ${userError instanceof Error ? userError.message : 'Unknown error'}`);
        }
  ...
  async getCurrentUser(): Promise<AuthUser | null> {
    try {
      ...
      if (!profile) {
        console.warn(`No Firestore profile found for authenticated user: ${user.uid}`);
        return null;
      }
