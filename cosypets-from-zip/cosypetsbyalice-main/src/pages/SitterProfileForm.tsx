...
import { db, dbHelpers, storage } from '../lib/firebase';
import { ref, uploadBytes, getDownloadURL } from 'firebase/storage';
import { collection, query, where, getDocs, doc, setDoc, deleteDoc, addDoc, orderBy } from 'firebase/firestore';
...
export default function SitterProfileForm() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const prof = await dbHelpers.getUserByAuthId(user.id) as any;
        if (prof) {
        ...
        // Load sitter profile
        const s = await dbHelpers.getSitterProfile(user.id) as any;
        if (s) {
        ...
        // media from firestore collection 'sitter_media'
        const mediaRef = collection(db, 'sitter_media');
        const qMedia = query(mediaRef, where('sitter_id', '==', user.id), orderBy('created_at', 'asc'));
        const mediaSnap = await getDocs(qMedia);
        if (!mediaSnap.empty) {
          setExistingMedia(mediaSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })));
        }

        // busy days from 'availability' collection
        const days = generateNext3Months();
        const availRef = collection(db, 'availability');
        const qAvail = query(availRef, where('sitter_id', '==', user.id), where('day', '>=', days[0]), where('day', '<=', days.slice(-1)[0]));
        const availSnap = await getDocs(qAvail);
        const map: Record<string, boolean> = {};
        availSnap.forEach((doc: any) => {
          const data = doc.data();
          map[data.day] = !!data.busy;
        });
        setBusyDays(map);
      } catch (err) {
  ...
  const saveProfile = async () => {
    if (!user?.id) return alert('Влез в профила си.');
    ...
    try {
      ...
      if (avatarFile) {
        const path = `avatars/${user.id}_${Date.now()}_${avatarFile.name}`;
      ...
      for (const f of toUpload) {
        const path = `sitter-media/${user.id}/${Date.now()}_${f.name}`;
        const storageRef = ref(storage, path);
        await uploadBytes(storageRef, f);
        const url = await getDownloadURL(storageRef);

        await addDoc(collection(db, 'sitter_media'), {
          sitter_id: user.id,
          url: url,
      ...
      for (const d of days) {
        ...
        const availRef = doc(db, 'availability', `${user.id}_${d}`);
        await setDoc(availRef, { sitter_id: user.id, day: d, busy }, { merge: true });
      }
  ...
  const removeMedia = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, 'sitter_media', id));
      setExistingMedia(x => x.filter(m => m.id !== id));
    } catch (err: any) {
